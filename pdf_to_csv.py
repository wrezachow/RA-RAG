...
def clean_text(s: str) -> str:
    ...
    s = re.sub(r"-\n(\w)", r"\1", s)            # fix broken hyphen linewraps
...
def pdf_to_csv(
    ...
    if len(rows) == 0:
        print("Warning: extracted 0 text chunks. This PDF might be scanned (image-only).")

    return out_csv

if __name__ == "__main__":
    pdf = input("PDF path: ").strip()
    out = input("Output CSV: ").strip()
    print("Saved to:", pdf_to_csv(pdf, out))
